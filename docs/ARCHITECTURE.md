...
| --- | --- | --- |
| مهمان محصول آرا‌لند | دیدن داشبورد نمونه و سه قالب، ورود با شماره موبایل | محتوای نمونه از داده واقعی جداست؛ تغییر پایدار نیازمند ورود است. |
...
| `Site` | نام، slug، قالب، پیش‌نویس، snapshot انتشار، SEO | slug سراسری یکتا؛ وابسته به یک فضای کاری |
...
۲. ویرایش مالک، `draft` و تنظیمات نام/SEO را تغییر می‌دهد. ساختار بخش‌ها شامل شناسه، نوع، وضعیت نمایش، عنوان، زیرعنوان و آیتم‌هاست. سرور شناسه‌های تکراری و URL نامعتبر را رد می‌کند.
۳. انتشار، محتوا، شناسه قالب، نام سایت و SEO را در `published`، `publishedTemplateId`، `publishedName` و `publishedSeo` کپی می‌کند. سایت `PUBLISHED` و زمان انتشار ثبت می‌شود.
۴. پاسخ عمومی فقط snapshot منتشرشده را برمی‌گرداند. برای سازگاری نوع مشترک، فیلد `draft` پاسخ عمومی نیز به محتوای منتشرشده اشاره می‌کند؛ پیش‌نویس واقعی و شناسه فضای کاری ارسال نمی‌شود.
۵. تغییر قالب در پیاده‌سازی فعلی **پیش‌نویس را با محتوای اولیه قالب جایگزین می‌کند** و نام سایت را نگه می‌دارد؛ snapshot قبلی عمومی باقی می‌ماند. حفظ خودکار تمام محتوای سفارشی در تغییر قالب هنوز وجود ندارد و رابط باید این اثر را پیش از عمل روشن کند.
...
| تجربه ساخت سایت | سه قالب واقعی، schema بخش و snapshot انتشار | آزمون جریان کامل وب/گوشی، کنترل تعارض، نسخه‌ها و حفاظت از تغییر قالب |
