...
هر پنج قالب از همان `SiteContent` و snapshot مشترک استفاده می‌کنند. هر قالب در `apps/web/src/components/templates` ترکیب‌بندی مستقل hero، services و about دارد؛ منطق فرم، استوری و لینک‌ها مشترک است و ظاهر بخش‌های تکمیلی و منوها با هویت قالب هماهنگ می‌شود. متن منوی سفارشی مالک بدون جایگزینی باقی می‌ماند. CSS هر هویت با کلاس قالب محدود شده و در ترتیب مشخص از `globals.css` وارد می‌شود. تصاویر اصلی محلی از origin وب دریافت می‌شوند؛ در موبایل این origin با `EXPO_PUBLIC_WEB_URL` تعیین می‌شود. قالب تازه با `createInitialContent` ساخته می‌شود و نظر یا نتیجه ساختگی ندارد. [برنامه بازطراحی قالب‌ها](TEMPLATE-IDENTITY-PLAN.md).
